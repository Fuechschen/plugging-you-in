...
class Client extends EventEmitter {
    ...
    _getCsrf() {
        return new Promise((resolve, reject) => {
            this._requestAgent.request(Constants.methods.get, Constants.endpoints.csrf).then(body => resolve(body.data[0].c)).catch(reject);
        });
    }
    ...
    unbanUser(userID) {
        return new Promise((resolve, reject) => {
            if (!this.ready)return reject(new Error('Client is not ready yet.'));
            this._requestAgent.request(Constants.methods.post, Constants.endpoints.bans + userID).then(resolve, reject);
        });
    }
